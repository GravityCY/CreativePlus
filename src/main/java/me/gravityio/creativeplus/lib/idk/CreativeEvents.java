...
public class CreativeEvents {
    ...
    public static final Event<OnMouseMoved> ON_MOUSE_MOVED = EventFactory.createArrayBacked(OnMouseMoved.class,
        listeners -> (window, x, y) -> {
            for (OnMouseMoved listener : listeners) {
                ActionResult result = listener.onMouseMoved(window, x, y);
    ...
    public static final Event<OnMouseMoved> ON_MOUSE_DELTA = EventFactory.createArrayBacked(OnMouseMoved.class,
            listeners -> (window, x, y) -> {
                for (OnMouseMoved listener : listeners) {
                    ActionResult result = listener.onMouseMoved(window, x, y);
                    if (result != ActionResult.PASS)
    ...
    public static final Event<OnMouseScrolled> ON_MOUSE_SCROLLED = EventFactory.createArrayBacked(OnMouseScrolled.class,
    ...
    public static final Event<OnMousePressed> ON_MOUSE_PRESSED = EventFactory.createArrayBacked(OnMousePressed.class,
    ...
    public static final Event<OnMousePressedAfter> ON_AFTER_MOUSE_PRESSED = EventFactory.createArrayBacked(OnMousePressedAfter.class,
        listeners -> (window, button, action, mods) -> {
     ...
     */
    public static final Event<OnItemUse> ON_ITEM_USE = EventFactory.createArrayBacked(OnItemUse.class,
    ...
    public interface OnMouseMoved {
        ActionResult onMouseMoved(long window, double x, double y);
    }
