 ...
 */
public class MoveEntityInputHandler implements DefaultInputListener {

    private final ClientEntityTeleportHandler teleportHandler;
    private final MinecraftClient client;
    public MoveEntityInputHandler(MinecraftClient client) {
        this.client = client;
    ...
    public ActionResult onMouseButton(long window, int button, int action, int mods) {
        ...
        if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT && isAltDown) {
            Entity targetedEntity = Helper.getTargetedEntity(InputTypeHelper.TARGETED_ENTITY, 20);
